...
public class ExceptionsLesson {
    ...
    public void getStackTrace(SomeService service, Logger log) {
        ...
        try {
            this.getStackTraceDeeper(service);
        }
        catch (Exception e) {
            StackTraceElement[] em = e.getStackTrace();
            String str = "";
            for (int i=0; i < em.length; i++ ) {
                str = str + em[i].toString();
            }
            log.log(str);
        }
    }
    ...
    public class HelloWorldException extends Exception {
        ...
        public HelloWorldException(String str, int i) {
            super(str);
            this.ErrorCode = i;
        }
    ...
    public String autocloseResource(Connection c, Logger log) {
        String data = null;
        try (Session os = c.createSession()){
            data = os.getData();
        }
        catch (IOException e) {
            log.log(e.getMessage());
